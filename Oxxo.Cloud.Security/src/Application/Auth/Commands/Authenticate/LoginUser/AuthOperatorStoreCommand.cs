    ...
    {      
        ...
        public async Task<AuthResponse> Handle(AuthOperatorStoreCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                StorePlace? store = await context.STORE_PLACE.Where(x => x.CR_STORE == request.UserDto.CrStore && x.CR_PLACE == request.UserDto.CrPlace && x.IS_ACTIVE).FirstOrDefaultAsync(cancellationToken) ?? throw new CustomException(GlobalConstantMessages.NOTRECORD, HttpStatusCode.UnprocessableEntity);
                OperatorStoreLink? obj = await context.OPERATOR_STORE_LINK.Where(x => x.OPERATOR_ID == request.Operator.OPERATOR_ID && x.STORE_PLACE_ID == store.STORE_PLACE_ID && x.IS_ACTIVE).FirstOrDefaultAsync(cancellationToken);
                ...
                SessionToken? sessionToken = await context.SESSION_TOKEN.Where(x => x.SESSION_TOKEN_ID == obj.SESSION_TOKEN_ID && x.GUID == request.Operator.OPERATOR_ID && x.IS_ACTIVE).FirstOrDefaultAsync(cancellationToken);
