...
namespace Oxxo.Cloud.Security.Application.Auth.Commands.Authenticate.LoginUser
{
    public class AuthenticateUser : IAuthenticate
    {
        ...
        {
            ApiKey? externalApp = await context.API_KEY.AsNoTracking().Include(x => x.EXTERNAL_APPLICATION).Where(x => x.EXTERNAL_APPLICATION != null && x.EXTERNAL_APPLICATION.CODE == GlobalConstantHelpers.INTERNALAPPNAME && x.IS_ACTIVE).OrderByDescending(x => x.EXPIRATION_TIME).FirstOrDefaultAsync(cancellationToken);
            if (externalApp == null)
            {
        ...
        private async Task<AuthResponse?> ValidateProcessExistAndRegresToken(Guid operadorId, string identification, CancellationToken cancellationToken)
        {
            if (await authenticateQuery.ValidateSessionActive(operadorId))
            {
                ...
                if (auhtResponse.Exp != null && await authenticateQuery.ValidateExpirationToken(auhtResponse.Token))
                {
                    return auhtResponse;
                }
                else
