...
public class AuthenticateQuery : IAuthenticateQuery
{
    ...
    {
        return await context.DEVICE.AsNoTracking().AnyWithNoLockAsync(x => x.NAME == nameDevice && x.IS_ACTIVE && x.DEVICE_STATUS.CODE != GlobalConstantHelpers.DEVICESTATUSDEPRECATED);
    }
    ...
    {
        var device = await context.DEVICE.AsNoTracking().FirstOrDefaultWithNoLockAsync(x => x.NAME == nameDevice && x.IS_ACTIVE && x.DEVICE_STATUS.CODE != GlobalConstantHelpers.DEVICESTATUSDEPRECATED);
        return (device != null) && await context.SESSION_TOKEN.AnyWithNoLockAsync(x => x.GUID == device.DEVICE_ID && x.IS_ACTIVE);
    }
    ...
    {
        return await context.SESSION_TOKEN.AnyWithNoLockAsync(x => x.GUID == idGuid && x.IS_ACTIVE);
    }
    ...
    {
        return await context.EXTERNAL_APPLICATION.AsNoTracking().AnyWithNoLockAsync(x => x.CODE == code && x.IS_ACTIVE);
    }
    ...
    {
        return await context.EXTERNAL_APPLICATION.AsNoTracking().FirstOrDefaultWithNoLockAsync(x => x.EXTERNAL_APPLICATION_ID == guidId,  cancellationToken);
    }
    ...
    {
        return await context.SESSION_TOKEN.AsNoTracking().AnyWithNoLockAsync(x => x.TOKEN == token);
    }
    ...
    /// <param name="token">The authentication token</param>
    /// <returns>Boolean value</returns>
    public async Task<bool> ValidateExpirationToken(string? token, CancellationToken cancellationToken)
    {
        return await context.SESSION_TOKEN.AsNoTracking().AnyWithNoLockAsync(x=> x.TOKEN == token && x.EXPIRATION_TOKEN >= DateTime.UtcNow && x.IS_ACTIVE, cancellationToken);
    }
    ...
    {
        return await context.SESSION_TOKEN.AsNoTracking().AnyWithNoLockAsync(x => x.REFRESH_TOKEN == refreshToken);
    }
    ...
    public async Task<string?> GetApiKeyExternalApplication(string? codeApplication, string ApiKey)
    {
        var result = await context.EXTERNAL_APPLICATION.AsNoTracking()
            ...
            .Select(x => x.APIKEYS.Where(w => w.IS_ACTIVE).ToList()).FirstOrDefaultWithNoLockAsync();
    ...
    {
        return await context.SESSION_TOKEN.Where(x => x.TOKEN == token).Select(x => x.REFRESH_TOKEN).FirstOrDefaultWithNoLockAsync();
    }
    ...
    public async Task<bool> ValidateExistUserPassword(string user, string password)
    {
        return await context.OPERATOR.AsNoTracking()
            ...
            .AnyWithNoLockAsync(x => x.USER_NAME == user && x.IS_ACTIVE && x.OPERATOR_PASSWORD.Any(x => x.PASSWORD == cryptographyService.EncryptHash(password) && x.IS_ACTIVE && x.EXPIRATION_TIME >= DateTime.UtcNow));
    }
    ...
    public async Task<SystemParam?> GetTimeExpirationTokenUser(CancellationToken cancellationToken) {
        return await context.SYSTEM_PARAM.AsNoTracking().FirstOrDefaultWithNoLockAsync(x => x.PARAM_CODE == GlobalConstantHelpers.TOKENEXPIRATIONUSER && x.IS_ACTIVE, cancellationToken);
    }
    ...
    {
        return await context.OPERATOR.AsNoTracking().Include(x => x.PERSON).AsNoTracking().FirstOrDefaultWithNoLockAsync(x => x.OPERATOR_ID == operadorId && x.IS_ACTIVE, cancellationToken);
    }
    ...
    public async Task<bool> ValidateExistAndActiveStorePlace(string crStore, string crPlace, CancellationToken cancellationToken) {
        return await context.STORE_PLACE.AsNoTracking().AnyWithNoLockAsync(x => x.CR_STORE == crStore && x.CR_PLACE == crPlace && x.IS_ACTIVE, cancellationToken: cancellationToken);
    }
    ...
    public async Task<Operator?> GetOperatorActiveForUser(string userName, CancellationToken cancellationToken)
    {
        return await context.OPERATOR.Include(x => x.PERSON)
           .FirstOrDefaultWithNoLockAsync(x => x.USER_NAME == userName && x.IS_ACTIVE, cancellationToken);
    }
    ...
    public async Task<List<SystemParam>> GetSystemParamsPasswordRules(CancellationToken cancellationToken)
    {
        ...
        var config = await context.SYSTEM_PARAM.ToListWithNoLockAsync(w => lstSystemParams.Contains((w.PARAM_CODE ?? "").ToUpper()), cancellationToken);
        return config;
    }
    ...
    public async Task<Operator?> GetOperatorActiveForUserPassword(string userName, string password, CancellationToken cancellationToken)
    {
        var list = await context.OPERATOR
         ...
         .ToListWithNoLockAsync(x => x.USER_NAME == userName && x.IS_ACTIVE, cancellationToken);


        return await context.OPERATOR
         .Include(i => i.OPERATOR_PASSWORD)
         .FirstOrDefaultWithNoLockAsync(x => x.USER_NAME == userName && x.IS_ACTIVE && x.OPERATOR_PASSWORD.Any(a => a.IS_ACTIVE && a.PASSWORD == password), cancellationToken);
    }
    ...
    public async Task<OperatorStatus> GetOperadtorStatus(string codeStatus, CancellationToken cancellationToken)
    {
        return await context.OPERATOR_STATUS
         .FirstOrDefaultWithNoLockAsync(x => x.CODE == codeStatus && x.IS_ACTIVE, cancellationToken);
    }
}