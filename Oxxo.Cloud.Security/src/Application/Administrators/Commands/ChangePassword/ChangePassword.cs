    ...
    public class ChangePassword : IChangePassword
    {
        ...
        public async Task<bool> ChangePasswordAdministratorAsync(ChangePasswordCommand request, CancellationToken cancellationToken)
        {
            ...
            OperatorPassword? operator_password = await context.OPERATOR_PASSWORD.Include(o => o.OPERATOR)
                      .Where(o => o.OPERATOR_ID == Guid.Parse(request.UserId) && o.PASSWORD == cryptographyService.EncryptHash(request.OldPassword)
                      && o.OPERATOR.IS_ACTIVE && o.IS_ACTIVE).FirstOrDefaultAsync(cancellationToken);
        ...
        {
            return await context.OPERATOR.AsNoTracking().Where(o => o.OPERATOR_ID == Guid.Parse(request.UserId) && o.IS_ACTIVE).AnyAsync(cancellationToken);
        }
        ...
        public async Task<bool> ValidateIfPasswordIsEqualToTheCurrentPassword(ChangePasswordCommand request, CancellationToken cancellationToken)
        {
            return await context.OPERATOR_PASSWORD.AsNoTracking().Include(o => o.OPERATOR)
                       .Where(o => o.OPERATOR_ID == Guid.Parse(request.UserId)
                        ...
                        && o.OPERATOR.IS_ACTIVE && o.IS_ACTIVE).AnyAsync(cancellationToken);
        }
        ...
        public async Task<bool> ValidateNewPassword(ChangePasswordCommand request, CancellationToken cancellationToken)
        {
            ...
            var config = await context.SYSTEM_PARAM.Where(w => codes_system_param.Contains((w.PARAM_CODE ?? string.Empty).ToUpper())).ToListAsync(cancellationToken);
        ...
        {
            int iDays = await context.SYSTEM_PARAM.AsNoTracking()
                        .Where(system_param => system_param.PARAM_CODE == GlobalConstantHelpers.PARAMEXPIRATIONTIME)
                        .Select(s => Convert.ToInt32(s.PARAM_VALUE)).FirstOrDefaultAsync(cancellationToken);
