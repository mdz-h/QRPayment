...
namespace Oxxo.Cloud.Security.Application.ExternalApps.Commands.UpdateExternalApps
{
    public class UpdateExternalAppsCommandValidator : AbstractValidator<UpdateExternalAppsCommand>
    {
        ...
        {
            return isValid = await context.EXTERNAL_APPLICATION.AsNoTracking().AnyAsync(x => x.EXTERNAL_APPLICATION_ID.ToString() == externalApp.ExternalAppId, cancellationToken);
        }
        ...
        { 
            if (isValid)
            {
                isValid = (externalApp.IsActive ?? false) || await context.EXTERNAL_APPLICATION.AsNoTracking().AnyAsync(x => x.EXTERNAL_APPLICATION_ID.ToString() == externalApp.ExternalAppId && x.IS_ACTIVE, cancellationToken);
            }
