...
namespace Oxxo.Cloud.Security.Application.ExternalApps.Commands.DeleteApiKey
{
    ...
    public class DeleteApiKeyCommandValidator : AbstractValidator<DeleteApiKeyCommand>
    {
        ...
        public async Task<bool> ValidateDeleted(DeleteApiKeyCommand command, CancellationToken cancellationToken)
        {
            if (isValid)
            {
                var entity = await context.API_KEY.Where(w => w.API_KEY == command.APIKey && w.EXTERNAL_APPLICATION_ID.ToString() == command.ExternalAppId && w.IS_ACTIVE).FirstOrDefaultAsync(cancellationToken);
                isValid = entity != null;
            }
        ...
        {
            return isValid = await context.API_KEY.AnyAsync(w => w.API_KEY == command.APIKey && w.EXTERNAL_APPLICATION_ID.ToString() == command.ExternalAppId, cancellationToken: cancellationToken);
        }
    }
}
