...
namespace Oxxo.Cloud.Security.Application.Device.Commands.Register
{
    public class RegisterDeviceCommandValidator : AbstractValidator<RegisterDeviceCommand>
    {
        ...
        {
            return this.validate = await context.STORE_PLACE.AsNoTracking().AnyWithNoLockAsync(x => x.CR_PLACE == crPlace && x.IS_ACTIVE, cancellationToken: cancellationToken);
        }
        ...
        private async Task<bool> ValidateCrStore(string? crStore, CancellationToken cancellationToken)
        {
            ...
            return this.validate = await context.STORE_PLACE.AsNoTracking().AnyWithNoLockAsync(x => x.CR_STORE == crStore && x.IS_ACTIVE, cancellationToken: cancellationToken);
        }
        ...
        private async Task<bool> ValidateTillNumber(int tillNumber, CancellationToken cancellationToken)
        {
            ...
            return this.validate = await context.DEVICE_NUMBER.AsNoTracking().AnyWithNoLockAsync(x => x.NUMBER == tillNumber && x.IS_ACTIVE, cancellationToken: cancellationToken);
        }
        ...
        private async Task<bool> ValidateDeviceType(string? deviceType, CancellationToken cancellationToken)
        {
            ...
            return this.validate = await context.DEVICE_TYPE.AsNoTracking().AnyWithNoLockAsync(x => x.CODE == deviceType && x.IS_ACTIVE, cancellationToken: cancellationToken);
        }
        ...
        private async Task<bool> ValidateIP(string? ip, CancellationToken cancellationToken)
        {
            ...
            return this.validate = await context.VALID_IP_RANGE.AsNoTracking().AnyWithNoLockAsync(x => x.IP_RANGE == ipRange && x.IS_ACTIVE, cancellationToken: cancellationToken);
