    ...
    public class RegisterDeviceHandler : IRequestHandler<RegisterDeviceCommand, bool>
    {
        ...
        private async Task<bool> GetDeviceUpdate(RegisterDeviceCommand request, int storePlaceId, int deviceTypeId, int deviceNumberId, CancellationToken cancellationToken)
        {
            ...
            var device = await context.DEVICE
                                 ...
                                 && x.DEVICE_STATUS.CODE != GlobalConstantHelpers.DEVICESTATUSDEPRECATED).FirstOrDefaultAsync(cancellationToken: cancellationToken);
        ...
        {
            var defaultWorkgroup =  await context.WORKGROUP.AsNoTracking().Where(x=>x.CODE == GlobalConstantHelpers.DEFAULTDEVICEWORKGROUPCODE && x.IS_ACTIVE).FirstOrDefaultAsync(cancellationToken: cancellationToken);
